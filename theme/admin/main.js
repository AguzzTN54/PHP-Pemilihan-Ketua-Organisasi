var animasi =
  ...
  modal =
  '<div class="modal-content wow zoomIn" data-wow-duration="0.3s"><div class="modal-header"><h5 class="modal-title"></h5>' +
  close +
  '</div><div class="modal-body"></div><div class="modal-footer"></div></div>',
  navbar =
...
for (let i = 0; i < menu.length; i++) {
  ...
  item += '<a data-href="./' + menu[i] + '.html" data-title="' + judul[i] + '" onclick="pushState(\'state' + [i] + '\');" id="state' + [i] + '" class="menu-item' + aktif + ' wow  fadeInUp" data-wow-delay="0.' + [i * 2] + 's">  ' + '<i class="fa fa-' + icon[i] + '"></i> <span>' + menuTitle + "</span> </a>";
}
...
function loadDashboard() {
  ...
  $(".app").load("dashboard.html", function (responseTxt, statusTxt, xhr) {
    if (statusTxt == "error") {
      notif(
        "danger",
        'Gagal memuat Aplikasi, <b>"<span>' +
        xhr.status +
        "</span> <span>" +
        xhr.statusText +
        '</span>"</b>',
...
function pushState(elementID) {
  ...
  $("#item-preload").fadeIn("slow", function () {
    $(this).show();
    $('.app').load(page, function (responseTxt, statusTxt, xhr) {
      if (statusTxt == "error") {
        notif("danger", 'Gagal memuat Aplikasi, <b>"<span>' + xhr.status + "</span> <span>" + xhr.statusText + '</span>"</b>', false);
      } else {
...
function refreshData() {
  ...
  sid = "";
  $.ajax({
    url: "dashboard.json?date=" + new Date().getTime(),
    dataType: "json",
...
(function ($) {
  ...
  window.onpopstate = function (e) {
    ...
    if (e.state.id) {
      $("#item-preload").fadeIn("slow", function () {
        $(this).show();
        $('.app').load(e.state.hal, function (responseTxt, statusTxt, xhr) {
          if (statusTxt == "error") {
            notif(
              "danger",
              'Gagal memuat Aplikasi, <b>"<span>' +
