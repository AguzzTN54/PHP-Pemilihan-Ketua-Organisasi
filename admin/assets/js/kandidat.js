function tambahAksi(target = null, aksi = null, id = null, nama = null) {
  ...
  if (target == null || aksi == null) {
    ...
    notif('danger', 'Tidak ddapat memuat');
    ...
    $('.modal-title').html('<span style="text-transform:capitalize">' + aksi + ' ' + target + ' ' + nama + '</span>');
    if (updateAksi) {
      $('.updateAksi').load('app/' + target + '/' + aksiURI, function (responseTxt, statusTxt, xhr) {
        if (loadMsg(statusTxt, xhr)) {
          if ($("#spiner").length) {
            $("#spiner").delay(500).fadeOut("slow", function () {
              $(this).remove();
            });
          }
        }
...
function hapusAksi(target, id, nama = null) {
  ...
  var idKandidat = id,
    button = '<button class="btn btn-primary" data-dismiss="modal">Batal</button> <button class="btn btn-danger hapus-fix" onclick="hapusFix(\'' + target + '\',' + idKandidat + ')">Hapus</button>';
  $('.modal-title').html('Yakin Menghapus ' + nama + ' ?');
  $('.modal-body').html('Yakin menghapus kandidat <b>' + nama + '</b> ? <br/> data yang dihapus tidak dapat dikembalikan !</div><div class="modal-footer">' + button);
  return;
}
...
function hapusFix(target = null, id = null) {
  if (target != null || id != null) {
    ...
    $('.hapus-fix').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Hapus');
    $.ajax({
...
$(document).ready(function () {
  ...
  $('.lihat').on('click', function () {
    ...
    $('.updateAksi').load('app/hasil/kandidat/' + idKandidat, function (responseTxt, statusTxt, xhr) {
      if (loadMsg(statusTxt, xhr)) {
        if ($("#spiner").length) {
